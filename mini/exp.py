...
def edit_data(id):  # display to edit and update record
    ...
    def my_update():  # update record
        ...
        print(type(data[0]))
        id = my_conn.execute(
            "UPDATE practice SET name='{1}', email='{2}' WHERE id={0}".format(data[0], data[1], data[2]))
...
def add_data():
    ...
    def my_add():
        ...
        adding = my_conn.execute(
            "INSERT INTO practice (id, name, email)VALUES ({0}, '{1}', '{2}')".format(data[0], data[1], data[2]))
